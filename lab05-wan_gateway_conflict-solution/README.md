...
`UserConflictHandler` decides how to reconcile a `User` record that both sites wrote independently before they were ever connected. The policy: conflicts sourced from EMEA are discarded, and conflicts sourced from US always override the local EMEA value, so US consistently wins.
...
- `ConflictResolution` - `UserConflictHandler`, which implements `onDataConflict()`.
...
- `BillBuddyGateway` - the WAN gateway module (delegator, sink, and the conflict resolver bean), deployed once per site with `-p` overrides.
...
```bash
# 4. Deploy the GATEWAY PUs - this is when the conflicts actually surface and resolve
docker compose --profile gateway up -d
```

## Verify the resolution

Grep both gateways' logs for the resolution outcome. US should show `ABORT` for every EMEA-sourced conflict; EMEA should show `OVERRIDE` for every US-sourced conflict.
