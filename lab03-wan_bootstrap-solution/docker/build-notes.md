...
`AdminBootstrapInitiator.java` waits internally (`admin.getGateways().waitFor("US")`) for the US gateway's sink to see EMEA, then calls `bootstrapFromGatewayAndWait()`. It reads its manager locator from the `GS_LOCATORS` environment variable, the same convention as the feeder's `GS_LOCATORS`/`SPACE_NAME`.
