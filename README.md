...
Training material for GigaSpaces XAP's WAN Gateway feature, covering six scenarios: one-directional and bidirectional replication, bootstrapping, replication filtering, conflict resolution, and adding/removing gateway targets at runtime. Each solution lab below is an independently self-contained Maven reactor with its own Docker Compose setup - see a lab's own README.md for its specific goals, topology, and run instructions.
...
| 3 | `lab03-wan_bootstrap-solution`                                               | WAN Gateway bootstrapping                              | pulling one site's existing data into another after the fact, Admin API                                                  |
...
| 6 | `lab06-wan_gateway_modify_target`                                            | managing WAN Gateway targets at runtime                | adding and removing WAN Gateway targets, Admin API, @Configuration based dependency injection (gateway module) |
| 7 | `lab10-active_active_on_nodes`                                               | the active-active topology deployed onto two real hosts | GigaSpaces blueprint, SpaceDocument, no Docker                                                                           |
