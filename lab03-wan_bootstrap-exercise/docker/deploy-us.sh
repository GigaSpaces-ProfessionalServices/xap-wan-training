...
echo "Deploying US gateway (BillBuddyGateway.jar, requires-bootstrap=true)..."
/opt/gigaspaces/bin/gs.sh --server=us-manager service deploy \
  --zones=US-gateway \
  -p localGatewayName=US \
  -p remoteGatewayName=EMEA \
  -p localSpaceUrl=jini://*/*/wanSpaceUS \
  -p requiresBootstrap=true \
...
echo "US deployment complete. US will NOT have EMEA's data until"
echo "'docker compose --profile bootstrap run --rm bootstrap-initiator' is run."
