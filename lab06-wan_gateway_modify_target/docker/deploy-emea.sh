...
echo "Deploying EMEA space (BillBuddySpace.jar, no gateway targets configured yet)..."
...
echo "Deploying EMEA gateway (BillBuddyGateway.jar, delegator+sink connected to US)..."
/opt/gigaspaces/bin/gs.sh --server=emea-manager service deploy \
  --zones=EMEA-gateway \
  -p localGatewayName=EMEA \
...
echo "EMEA deployment complete. The gateway is connected to US, but wanSpaceEMEA"
echo "has no active outbound gateway target either -- this lab only ever adds/removes"
echo "the US-to-EMEA direction, EMEA's own target list is never touched."
