...
# Run this on the node designated as siteB ({{siteB.gatewayName}}).
...
export GS_MANAGER_SERVERS="$SITE_B_HOST"
...
export GS_GSC_OPTIONS="$ORIGINAL_GS_GSC_OPTIONS -Dcom.gs.zones=${SITE_B_NAME}-space"

nohup $GS_HOME/bin/gs.sh host run-agent --manager --gsc=4 > /tmp/space-${SITE_B_NAME}.log 2>&1 &
...
export GS_GSC_OPTIONS="$ORIGINAL_GS_GSC_OPTIONS \
  ...
  -Dcom.gs.zones=${SITE_B_NAME}-gateway"

nohup $GS_HOME/bin/gs.sh host run-agent --gsc=1 > /tmp/gateway-${SITE_B_NAME}.log 2>&1 &
