# lab04-wan_gateway_filter-exercise - WAN Gateway Filter
...
- `ReplicationFilter/src/main/java/.../ReplicationFilter.java` -- the `process()` method is empty. Search for `TODO` in this file: it should discard any `User` write bound for `gateway:EMEA` unless the user's `location` is `Continent.Europe`.
- `BillBuddySpaceUS/src/main/resources/META-INF/spring/pu.xml` -- the filter bean's `class` and the `output-filter`'s `ref` are both left as the placeholder value `TODO`. Search for `TODO` in this file and wire them to the real filter bean.

Until both are filled in, US's space PU won't deploy (Spring can't resolve a class literally named `TODO`), so you'll want to fix these before step 2 below -- the rest of the lab (build, deploy, feed) is there to follow once they're wired up.

## Lab Description

`ReplicationFilter` is attached to US's space and inspects every `User` object about to replicate to EMEA. It should discard the write unless the user's `location` is `Continent.Europe`; every other object type, and everything replicating in the EMEA to US direction, is unaffected. Both sites otherwise run a full delegator and sink, so this is bidirectional active-active replication with one direction selectively filtered.
...
- `ReplicationFilter` - the filter implementation, attached only to US's space. `process()` should discard any `User` write bound for EMEA unless `location == Continent.Europe` -- this is one of the two TODOs above.
...
- `BillBuddySpaceUS` - the space module carrying the filter, wired in via `<os-core:space-replication-filter>` -- the other TODO above.
...
Once both TODOs are filled in: EMEA should only have the Europe subset of the Users fed into US; every other object type replicates in full.
